...
import { ProfesionService } from 'src/app/core/services';
...
export class VistaProfesionComponent implements OnInit {
  ...
  constructor(
      private _router: Router, private _route: ActivatedRoute, private _profesionService: ProfesionService, private _toastrService: ToastrService
    ) {
  }
  ...
  /**
    ...
    guardar(datos:any) {
      if (datos !== false){
        if (datos["id"] == 0){
          this._profesionService.guardar(datos, 0).subscribe(
            respuesta => {
              this._toastrService.success('Se ha creado un nuevo oficio!!!');
              this.refrescarListado();
            }, error => { this._toastrService.error(error); });
        }else{
          this._profesionService.guardar(datos, datos["id"]).subscribe(
            respuesta => {
     ...
     */
    borrar(id:number) {
      this._profesionService.borrar(id).subscribe(
        respuesta => {
